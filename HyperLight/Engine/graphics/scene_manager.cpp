	...
	: m_frustum( nullptr )
{
	...
	m_multi_tex	= new CMultiTextureShader();
}
...
bool	CSceneManager::Render( ID3D11DeviceContext* context, D3DXMATRIX world, D3DXMATRIX view, D3DXMATRIX projection )
{
	...
	for( it = m_assets.begin(); it != m_assets.end(); it++ )
	{
		if ( false || m_frustum->CheckSphere( it->Position, radius ) > Frustum::OUTSIDE )
		{
			...
			if ( m_multi_tex->SetShaderParameters( context, world, view, projection, m_meshs[ it->MeshID ]->textures->data, m_meshs[ it->MeshID ]->textures->count ) )
			{
				m_multi_tex->Render( m_meshs[ it->MeshID ]->IndexCount );
			}
